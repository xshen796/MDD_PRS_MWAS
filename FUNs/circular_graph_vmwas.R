...
circular_graph_vmwas <- function(target.res,output.file=NA,target.col='P.value'){
  target.result = target.res
  # create a data frame giving the hierarchical structure of your individuals
  set.seed(1234)
  d1 <- data.frame(from="origin", to=c('SNP',
                                       'CpG'))
  d2 <- data.frame(from='CpG', to=unique(target.result$CpG))
  d3 <- data.frame(from='SNP', to=unique(target.result$SNP))
  hierarchy <- rbind(d1, d2, d3)
  
  
  connect <- target.result %>%
    select(from=CpG,to=SNP,value=log.p.val)
  ...
  vertices$angle <- 160 + 3.3*nrow(target.result) - 360 * vertices$id / nleaves
  ...
  p.label = p + 
    ...
    scale_colour_manual(values= c('lightcoral','dodgerblue3'),name = 'CpG/SNP') +
