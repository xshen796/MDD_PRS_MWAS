...
outcome.summstats=readRDS(summ.outcome) %>%
   dplyr::select(cpg,SNP,beta_a1,se,pval,samplesize,allele1,allele2,freq_a1)
colnames(outcome.summstats)=c('CpG','SNP','BETA','SE','P','N','Allele1','Allele2','af')
...
for (f in 1:length(ls.exposure)){
      ...
      if (count.instrument>=5){
         outcome_match=outcome.summstats[outcome.summstats$SNP %in% exposure.dat$SNP,] %>%
            dplyr::select(SNP,Allele1,Allele2,af,BETA,SE,P)
         colnames(outcome_match)=c("SNP","effect_allele","other_allele","eaf","beta","se","pval")
         outcome_match$Phenotype=traitB
         outcome_match$samplesize=9213
         
         write.table(outcome_match,
                     file=paste0(output.path,'/',traitA,'.',traitB,".outcome_dat"),
