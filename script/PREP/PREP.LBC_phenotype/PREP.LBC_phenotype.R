...
# PRS
LBCboth.PRS=read.table('data/LBC_phenotype/MDDprs/MDDprs.LBCboth.imputed.all_score',header=T,stringsAsFactors = F)
LBCboth.PRS.tophits=read.table('data/LBC_phenotype/MDDprs/MDDprs.imputed.topSNP.all_score',header=T,stringsAsFactors = F)
...
LBC.meth.tech.covar = rbind(LBC21.tech,LBC36.tech) %>%
  select(Basename,WAVE,ID,age,sex,cohort,plate) %>%
...
# pT PRS
LBC.PRS.pt = LBCboth.PRS %>%
  select(IID,matches('Pt'))
LBC.PRS.tophits = LBCboth.PRS.tophits %>%
  select(IID,matches('Pt'))
...
saveRDS(linkage.file,file='data/LBC_phenotype/sentrix.rds')
