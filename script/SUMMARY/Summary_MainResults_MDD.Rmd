...
for (pt in ls.pt){
    ...
    tmp.path = paste0('Figs/GS_EWAS_pplot_',pt,'.png')
...
fig.5e_8=m_plot(dat=fig.dat.GS.pT.0.00000005,
                    ...
                    outputpath=here::here('Figs/GS_EWAS_pplot_0.00000005.png'))
```

#### PRS pT=5e-8 no MHC region

```{r,echo=FALSE,warning=FALSE,message=FALSE,fig.width=13,fig.height=7}
    f.path=paste0('result/EWAS_MDDprs_Shen/noMHC_and_gwsig/ewas_BothWaves_noMHC_5e_08.metal.out1.tbl')
    ewasResults=read.delim(here::here(f.path),sep='\t',header=T,stringsAsFactors=F) %>%
      mutate(p.adj = p.adjust(P.value,method='bonferroni')) 
    colnames(ewasResults)[1]='CpG'
    # Add annotations
   ...
   fig.GSewas.5e_08.noMHC = m_plot(dat=qman.dat,
          ...
          outputpath=here::here('Figs/GS_EWAS_pplot_5e_08_noMHC.png'))
...
ggsave(plot = fig,filename = here::here('Figs/No_sig_CpG_GS.png'),
       width = 12, height = 9, 
       units = 'cm', dpi = 300)
...
ggsave(plot = fig,filename = here::here('Figs/Fig1.png'),
       width = 12, height = 9, 
       units = 'cm', dpi = 300)
...
load(here::here('result/EWAS_MDDprs_fromKathryn/ewas.distance.RData'))
...
fig.distance_to_nearest_gwasHit
# Save
ggsave(file=here::here('Figs/MainText/Distance_to_nearest_gwasHit.png'),
...
write.table(GO.result.sig[1:10,],file=here::here('result/EWAS_MDDprs_Shen/GO_KEGG/GOterms.txt'),quote=F,sep='\t',col.names = T,row.names = T)
```
...
KEGG.result.sig[1:10,] %>%
  kbl() %>%
  kable_material(c("striped", "hover"))
write.table(KEGG.result.sig[1:10,],file=here::here('result/EWAS_MDDprs_Shen/GO_KEGG/KEGG.txt'),quote=F,sep='\t',col.names = T,row.names = T)
```
...
GO.result.sig_noMHC[1:20,] %>%
  kbl() %>%
  kable_material(c("striped", "hover"))
write.table(GO.result.sig_noMHC[1:10,],file=here::here('result/EWAS_MDDprs_Shen/GO_KEGG/GOterm_noMHC.txt'),quote=F,sep='\t',col.names = T,row.names = T)
```
...
KEGG.result.sig_noMHC[1:20,] %>%
  kbl() %>%
  kable_material(c("striped", "hover"))
write.table(KEGG.result.sig_noMHC[1:10,],file=here::here('result/EWAS_MDDprs_Shen/GO_KEGG/KEGG_noMHC.txt'),quote=F,sep='\t',col.names = T,row.names = T)
```
...
write.table(top.10.sig[1:10,],file=here::here('result/EWAS_MDDprs_Shen/top_CpG_withMHC.txt'),quote=F,col.names = T,row.names = F,sep='\t')
...
write.table(top.10.sig.noMHC[1:10,],file=here::here('result/EWAS_MDDprs_Shen/top_CpG_noMHC.txt'),quote=F,col.names = T,row.names = F,sep='\t')
...
fig.rep.pplot=m_plot(dat=fig.dat,chr="CHR", bp="MAPINFO", snp="CpG", p="P.Value",tophits.bpwindow=250000,
       outputpath=here::here('Figs/MainText/REP_EWAS_pplot_5e_08.png'),
...
ggsave(file=here::here('Figs/MainText/Replication_beta_correlation_GS_REP.png'),
       plot=fig,
       dpi = 300, width = 6, height = 4, units = 'in')
```
...
ggsave(plot = fig,filename = here::here('Figs/No_sig_CpG_LBC.png'),
       width = 12, height = 9, 
       units = 'cm', dpi = 300)

fig3.inpaper = ggarrange(fig.rep.pplot,
                         ggarrange(fig.rep.nsig,fig.corr.rep,ncol=2,
                                   labels=c('b','c'),align ='v'),
                         nrow = 2,labels = 'a',heights = c(1,0.7))

ggsave(plot = fig3.inpaper,filename = here::here('Figs/Fig3.png'),
       width = 30, height = 25, 
       units = 'cm', dpi = 300)
...
fig.rep.adolescent.pplot=m_plot(dat=fig.dat,chr="CHR", bp="MAPINFO", snp="CpG", p="P.Value",tophits.bpwindow=250000,
       outputpath=here::here('Figs/MainText/REPadolescent_EWAS_pplot_5e_08.png'),
       add_category_name=F,fig_size=c(22,13),tophits_annot=T,y_lim=c(0,130))
...
ewasResult=read.delim('result/EWAS_MDDprs_Shen/MDDprs_ewas_meta/ewas_meta_pT_0.00000005.metal.out1.tbl')
anno <- getAnnotation(IlluminaHumanMethylationEPICanno.ilm10b2.hg19)
ref.tomerge <- anno %>% as.data.frame %>%
      dplyr::select(ID=Name, CHR=chr, MAPINFO=pos,ucsc_gene=UCSC_RefGene_Name,
                    Islands_Name,Relation_to_Island)
...
# Heatmap
set.seed(1234)
png(here::here('Figs/MainText/SNP_to_CpG_mapping.png'), height = 2600, width = 3800,res = 300)
...
# Reload data
load(here::here('result/EWAS_MDDprs_fromKathryn/cpg_snp_mapping_pNbeta.RData'))
...
summary.instrument=readRDS(here::here('data/mQTL/GoDMC.instrument.rds'))

# Load EWAS summary stats
ewas.summstat.tophitPRS=fread(here::here('result/EWAS_MDDprs_fromKathryn/meta-pgrs-mdd/mdd_SNPCH_prs_5e_08/mdd_SNPCH_prs_5e_08.metal.out1.tbl'),
...
-   `r length(list.files(here::here('data/mQTL/mQTL_forMR/')))` CpGs left after 'clumping' (window = 3Mb, r=0.1) in the MHC region.
...
# Load results
res = as.list(paste0('/gpfs/igmmfs01/eddie/GenScotDepression/shen/ActiveProject/Genetic/result/GoDMC_MR_DNAm_to_MDD/Summary_DNAm_to_',ls.measure,'.csv')) %>% 
...
ggsave(fig.total,device = 'png',height=15,width = 40,units = 'cm',dpi=300,
       filename = here::here('Figs/MR_DNAm_to_brain/MR_res.png'))
fig.total
...
# Load results
res = as.list(paste0('/gpfs/igmmfs01/eddie/GenScotDepression/shen/ActiveProject/Genetic/result/GS_MR_DNAm_to_MDD/Summary_DNAm_to_',ls.measure,'.csv')) %>% 
  lapply(.,FUN=read.csv,header=T,sep='\t',stringsAsFactors = F)
names(res)=ls.measure
```
...
ls.measure = list.files(paste0('/gpfs/igmmfs01/eddie/GenScotDepression/shen/ActiveProject/Genetic/result/GS_MR_MDD_to_DNAm/'),pattern = 'Summary') %>% 
  gsub('Summary_MDD_to_','',.) %>% 
  gsub('.csv','',.)

# Load results
res = list.files(paste0('/gpfs/igmmfs01/eddie/GenScotDepression/shen/ActiveProject/Genetic/result/GS_MR_MDD_to_DNAm/'),pattern = 'Summary',full.names = T) %>% 
  lapply(.,FUN=read.csv,header=T,sep='\t',stringsAsFactors = F)
names(res)=ls.measure
```
...
# Discovery analysis
res = as.list(paste0('/gpfs/igmmfs01/eddie/GenScotDepression/shen/ActiveProject/Genetic/result/GoDMC_MR_DNAm_to_MDD/Summary_DNAm_to_MDD.csv')) %>% 
  lapply(.,FUN=read.csv,header=T,sep='\t',stringsAsFactors = F)
names(res)='MDD'
mr.sig.output = res[[1]] %>%#
...
res = as.list(paste0('/gpfs/igmmfs01/eddie/GenScotDepression/shen/ActiveProject/Genetic/result/GS_MR_DNAm_to_MDD/Summary_DNAm_to_MDD.csv')) %>% 
  lapply(.,FUN=read.csv,header=T,sep='\t',stringsAsFactors = F)
names(res)='MDD'
fig.p.replication.mr = p.plot.mr(res,tmp.title='DNAm (GS) to MDD',rm.y = T)

# Reversed direction MR
res = as.list(list.files('/gpfs/igmmfs01/eddie/GenScotDepression/shen/ActiveProject/Genetic/result/GS_MR_MDD_to_DNAm/',pattern='^Summary',full.names = T)) %>% 
  lapply(.,FUN=read.csv,header=T,sep='\t',stringsAsFactors = F) %>%
...
ggsave(fig.total,device = 'png',height=9,width = 24,units = 'cm',dpi=300,
       filename = here::here('Figs/MR_res_pplot.png'))
fig.total
