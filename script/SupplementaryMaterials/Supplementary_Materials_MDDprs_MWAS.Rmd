...
discovery.summstats = list.files(here::here('MR_meth_MDD/result/EWAS_MDDprs_Shen/MDDprs_ewas_meta/'),
...
names(discovery.summstats) = list.files(here::here('MR_meth_MDD/result/EWAS_MDDprs_Shen/MDDprs_ewas_meta/'),
                                        recursive = T,pattern = 'ewas_meta',full.names = T) %>% 
    .[!grepl('.info$',.)] %>% 
    strsplit(.,'/') %>% 
...
summstats.5e_08 = list.files(here::here('MR_meth_MDD/result/'),
...
names(summstats.5e_08) = list.files(here::here('MR_meth_MDD/result/'),
                                   recursive = T,pattern = '5e|0.00000005',full.names = T) %>% 
  .[grep('MDD3_k_5e08|REPmeta_EWAS|ewas_meta_pT_0.00000005',.)] %>% 
...
nomhc.summstats = list.files(here::here('MR_meth_MDD/result/EWAS_MDDprs_Shen/noMHC_and_gwsig/'),
...
names(discovery.summstats) = list.files(here::here('MR_meth_MDD/result/EWAS_MDDprs_Shen/noMHC_and_gwsig/'),
                                        recursive = T,pattern = 'ewas_BothWaves',full.names = T) %>% 
    .[!grepl('.info$',.)] %>% 
    strsplit(.,'/') %>% 
    lapply(.,function(x) tail(x,1)) %>% 
...
```{r, warning=F,echo=F,fig.width=8.3,fig.height=8}
replication.summstats = list.files(here::here('MR_meth_MDD/result/EWAS_MDDprs_meta_LBC_ALSPAC/'),
...
names(replication.summstats) = list.files(here::here('MR_meth_MDD/result/EWAS_MDDprs_meta_LBC_ALSPAC/'),
                                        recursive = T,pattern = 'REPmeta',full.names = T) %>% 
    .[!grepl('.info$',.)] %>% 
    strsplit(.,'/') %>% 
    lapply(.,function(x) tail(x,1)) %>% 
...
#     return(ewas_res)
# }
youth.summstats = list.files(here::here('MR_meth_MDD/result/EWAS_MDDprs_ALSPAC/'),
...
names(youth.summstats) = list.files(here::here('MR_meth_MDD/result/EWAS_MDDprs_ALSPAC/'),
                                   recursive = T,pattern = 'MDD3_k_',full.names = T) %>% 
    .[grep('.csv',.)] %>% 
    .[!grepl('.info$|MDD3_k_102',.)] %>%  
    strsplit(.,'/') %>% 
...
![](/exports/igmm/eddie/GenScotDepression/shen/ActiveProject/Genetic/MR_meth_MDD/Figs/zoomlocus/zoomlocusplots.png){width="680" height="800"}
...
```{r, warning=F,echo=F,fig.width=8.3,fig.height=8.5}
# Load figure data
f.scatter = list.files(here::here('MR_meth_MDD/result/GoDMC_MR_DNAm_to_MDD/'),pattern = 'scatterplot.RData',recursive = T,full.names = T) 
...
mr.sig.discovery = readRDS(here::here('MR_meth_MDD/result/GoDMC_MR_DNAm_to_MDD/mr_SigOutput.rds'))
...
```{r, warning=F,echo=F,fig.width=8.3,fig.height=8.5}
# Load figure data
f.scatter = list.files(here::here('MR_meth_MDD/result/GS_MR_DNAm_to_MDD/'),pattern = 'scatterplot.RData',recursive = T,full.names = T) 
...
```{r,warning=F,echo=F,fig.width=8.3,fig.height=8.5}
# Load figure data
f.scatter = list.files(here::here('MR_meth_MDD/result/GS_MR_MDD_to_DNAm/'),pattern = 'scatterplot.RData',recursive = T,full.names = T) 
...
```{r,warning=F,echo=F}
PRS_predict_MDD = fread(here::here('MR_meth_MDD/result/MDDprs_predicion.txt')) %>% .[c(1:7,9:10),] %>% 
...
load(here::here('MR_meth_MDD/result/EWAS_MDDprs_Shen/Cell_proportion_MDDprs/Reg_res.RData'))
...
```{r, warning=F,echo=F}
table.lambda = read.table(here::here('MR_meth_MDD/result/lambda_MWAS_discovery_repAdult_repYouth.txt'),sep = '\t',header=T) %>% 
  mutate_if(is.numeric, round_if) %>% 
...
```{r,warning=F,echo=F}
table.sigcpg.withMHC=fread(here::here('MR_meth_MDD/result/EWAS_MDDprs_Shen/top_CpG_withMHC.txt')) %>% 
...
table.sigcpg.noMHC=fread(here::here('MR_meth_MDD/result/EWAS_MDDprs_Shen/top_CpG_noMHC.txt')) %>% 
  select(CpG,CHR,BP=MAPINFO,Beta=Effect,SE=StdErr,p=P.value,p.adj,Direction,HetChiSq,HetPVal) %>% 
...
```{r,warning=F,echo=F}
location_mat=read.delim(here::here('MR_meth_MDD/result/location_mat.txt'),header=T) 
...
mr.res = list.files(here::here('MR_meth_MDD/result/'),
...
```{r,warning=F,echo=F}
mvmr.res=fread(here::here('MR_meth_MDD/result/GS_MR_DNAm_to_MDD/Summary_DNAm_to_MDD_mvMR.txt'))
...
write.table(sig.res.MWAS,file=here::here('MR_meth_MDD/result/summary_sig_CpG_discovery_replication.csv'),sep='\t',col.names = T,row.names = F)
